...
{
    public class ExcelModuleOrleansTests
    {
        private IClusterClient client;

        private IExcelModuleOrleans excelModule;

        private string filesPath;

        public ExcelModuleOrleansTests()
        {
            var siloHost = new SiloHostBuilder().UseLocalhostClustering().Build();
            siloHost.StartAsync().GetAwaiter().GetResult();

            this.client = new ClientBuilder().UseLocalhostClustering().Build();
            this.client.Connect().GetAwaiter().GetResult();

            this.excelModule = this.client.GetGrain<IExcelModuleOrleans>(Guid.NewGuid());
